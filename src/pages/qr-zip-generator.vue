                  ...
                  <div class="col-sm row">
                    ...
                    <div class="col-sm-8">
                      <select v-model="qrImgSize" class="form-control">
                        <option v-for="qrImgSizeItem in qrImgSizeItems" :value="qrImgSizeItem" :key="qrImgSizeItem">{{ qrImgSizeItem }}</option>
                      </select>
                    </div>
                  </div>
                </div>
  ...
  methods: {
    ...
    convert() {
      if (!this.scriptLoaded) {
        loadAssets({ js: ["/assets/static/js/plugins/lzma_worker-min.js", "/assets/static/js/plugins/qrcode.min.js"] })
          .then(() => {
    ...
    createQRCode() {
      ...
      LZMA.compress(qrText, 6, (result, err) => {
        if (err) {
          this.errorMessage = "압축 에러: " + err;
          return;
        }
        try {
          const u8 = toUint8Array(result);
          const b64 = bytesToBase64(u8);
          let compressText = b64;

          let pagingQrTextMap = {};
          for (let i = 0; i < compressText.length; i++) {
            currChar = compressText.charAt(i);
          ...
          this.errorMessage = "결과 처리 에러: " + e.message;
        }
      });
    },
    qrImgDownload() {
